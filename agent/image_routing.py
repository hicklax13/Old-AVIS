...
# extract_local_files() uses, plus Windows drive-letter paths: anchors to
# ``~/``, ``/``, or ``C:\``/``C:/``, ignores matches inside URLs (the
# ``(?<![/:\w.])`` lookbehind — note it also keeps the ``s:`` in ``https://``
# from being read as a drive letter), and is case-insensitive on the extension.
